...
import { Plus, Minus, ImagePlus, ChevronLeft } from "lucide-react";
...
export default function AddProductPage() {
  ...
  useEffect(() => {
    ...
    return onSnapshot(q, snap => {
      setBrands(snap.docs.map(d => ({ id: d.id, name: d.data().name as string })).sort((a, b) => a.name.localeCompare(b.name)));
    });
  }, []);
  ...
  const selectProductFamily = async (item: ProductFamily) => {
    ...
    const loaded = snap.docs.map(d => ({ id: d.id, title: d.data().title, sortOrder: d.data().sortOrder ?? 999, specs: (d.data().specs || []).map((r: any) => ({ ...r, value: "" })) })).sort((a, b) => a.sortOrder - b.sortOrder);
  ...
  const handleSaveProduct = async () => {
    ...
    try {
      ...
      const productRef = await addDoc(collection(db, "products"), {
        ...
        productFamilies: [{ productFamilyId: selectedProductFamily.id, productFamilyName: selectedProductFamily.name, productUsageId: selectedCategoryTypes[0]?.id || "" }],
        ...
        createdBy: userId, referenceID: user?.ReferenceID || null,
        isActive: true, createdAt: serverTimestamp(), whatHappened: "Product Added", date_updated: serverTimestamp(),
      });
    ...
    <div className="h-screen overflow-hidden">
      ...
      <div className="md:hidden sticky top-0 z-20 bg-white border-b border-gray-100 shadow-sm">
        <div className="flex items-center justify-between px-4 h-14">
          ...
          <h1 className="text-sm font-bold text-gray-900">Add Product</h1>
