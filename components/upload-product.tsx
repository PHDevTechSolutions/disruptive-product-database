...
export default function UploadProduct({ iconOnly = false }: Props) {
   ...
   *
   * NEW column layout (matches download-product.tsx):
   *   Col 1  → Product Usage
   *   Col 2  → Product Family
   *   Col 3  → Product Class
   *   Col 4  → Price Point
   *   Col 5  → Brand Origin
   *   Col 6  → Supplier Brand
   *   Col 7  → Image URL
   *   Col 8+ → Technical Spec columns
   *   ...    → COMMERCIAL DETAILS group
   *              (Unit Cost, Length, Width, Height, pcs/carton,
   *               Factory Address, Port of Discharge)
   *   ...    → Dimensional Drawing   ← moved AFTER commercial details
   *   ...    → Illuminance Level   ← moved AFTER commercial details
   * ---------------------------------------------------------------- */
  const parseWorkbookRows = async (
    ...
    for (let wsIndex = 0; wsIndex < workbook.worksheets.length; wsIndex++) {
      ...
      for (let col = 1; col <= ws.columnCount; col++) {
        ...
        // ── Commercial detail sub-headers (row 3) ──
        ...
        // ── Skip the COMMERCIAL DETAILS group label row ──
        if (groupHeader === "COMMERCIAL DETAILS") continue;

        // ── Must have both group and spec header to be a spec column ──
      ...
      for (let r = 4; r <= ws.actualRowCount; r++) {
        ...
        // Static columns 1-7 (no dimensional/illuminance here anymore)
        ...
        // Drawing columns — now after commercial details
        const dimensionalURL = convertDriveToThumbnail(
          extractHyperlink(
            commercialColMap.dimensionalURL > 0
              ? row.getCell(commercialColMap.dimensionalURL).value
              : null,
          ),
        );
        const illuminanceURL = convertDriveToThumbnail(
          extractHyperlink(
            commercialColMap.illuminanceURL > 0
              ? row.getCell(commercialColMap.illuminanceURL).value
              : null,
          ),
        );
  ...
  const checkForDuplicates = async (
    ...
    const COLS = [
      ...
      "Brand Origin", "Supplier Brand", "Image",
