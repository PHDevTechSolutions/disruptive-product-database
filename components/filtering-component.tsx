...
export default function FilteringComponent({ products, onFilter }: Props) {
  ...
  const formatSpec = (s: any): string => {
    ...
    if (s.isSlashing)
      return s.slashValues?.join("/") || "";

    if (s.isDimension)
      return `${s.slashValues?.join(" x ") || ""}${s.unit ? ` ${s.unit}` : ""}`;

    if (s.isIPRating)
      return `IP${s.ipFirst}${s.ipSecond}`;
  ...
  const sisterCompanies =
    uniq(products.map((p) => p.sisterCompanyName));

  const classifications =
    uniq(products.map((p) => p.classificationName));
  ...
  const pricingFilters: Record<string, string[]> = {
    ...
    "Landed Cost":
      uniq(products.map((p) =>
        formatPHP(p.logistics?.landedCost, 2)))
        .filter((v) => v !== "-"),

    "SRP Cost":
      uniq(products.map((p) =>
        formatPHP(p.logistics?.srp, 0)))
        .filter((v) => v !== "-"),
  ...
  useEffect(() => {
    ...
    const filtered = products.filter((p) => {
      ...
      if (!check("Category Type", p.categoryTypes?.[0]?.categoryTypeName)) return false;

      if (!check("Product Type", p.productTypes?.[0]?.productTypeName)) return false;
      ...
      if (!check("SRP Cost", formatPHP(p.logistics?.srp, 0)))
        return false;

      if (!check("Category", p.logistics?.category || ""))
        return false;
      ...
      if (filters["Specification Mode"]?.length) {
        ...
        const ok =
          p.technicalSpecifications?.some((g: any) =>
            g.specs?.some((s: any) =>

              (filters["Specification Mode"].includes("Dimension") && s.isDimension) ||

              (filters["Specification Mode"].includes("Ranging") && s.isRanging) ||

              (filters["Specification Mode"].includes("Slashing") && s.isSlashing) ||

              (filters["Specification Mode"].includes("IP Rating") && s.isIPRating)
  ...
  return (
    ...
    <div className="border rounded-lg p-4 bg-card space-y-4">
      ...
      <div className="space-y-3 max-h-[500px] overflow-y-auto">
        ...
        <Section title="Sister Company" items={sisterCompanies} filters={filters} toggle={toggle} setSearch={setSearch} />

        <Section title="Classification Type" items={classifications} filters={filters} toggle={toggle} setSearch={setSearch} />

        <Section title="Category Type" items={categories} filters={filters} toggle={toggle} setSearch={setSearch} />

        <Section title="Product Type" items={productTypes} filters={filters} toggle={toggle} setSearch={setSearch} />

        <Section title="Supplier" items={suppliers} filters={filters} toggle={toggle} setSearch={setSearch} />
