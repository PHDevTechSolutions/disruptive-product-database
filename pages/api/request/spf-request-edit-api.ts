  ...
  try {
      ...
      try {
        ...
        if (snap.exists()) {
          ...
          supplierCache.set(supplierId, {
            company: d.company || "-",
            contactNames: contacts.map((c: any) => c.name).join(" | "),
            contactNumbers: contacts.map((c: any) => c.phone).join(" | "),
          });
        }
      } catch {}
    }
    ...
    for (let rowIdx = 0; rowIdx < rowCount; rowIdx++) {
      ...
      for (let i = 0; i < rowProducts.length; i++) {
        ...
        itemCodes.push(`${rowBase}-OPT-${i + 1}`);

        const cached = supplierCache.get(p?.supplier?.supplierId);
        companyNames.push(cached?.company || "-");
    ...
    const finalItemCode = rowItemCodes.join(ROW_SEP);
