...
export function QuotationSheet(props: Props) {
  ...
  const [localQuotationNumber, setLocalQuotationNumber] = useState<
    string | null
  >(null);
  ...
  async function fetchNextQuotationSequence(
    prefixBase: string,
  ): Promise<string> {
    const currentYear = new Date().getFullYear();
    ...
    try {
      const response = await fetch(
        `/api/fetch-quotation-number?prefix=${encodeURIComponent(prefixWithYear)}`,
      );
  ...
  useEffect(() => {
    ...
    const skus = selectedProducts.map((p) =>
      p.skus && p.skus.length > 0 ? p.skus[0] : "",
    );
    ...
    try {
      ...
      const initiateNewPage = async () => {
        const banner = await renderBlock(
          `<img src="${headerImagePath}" class="header-img" />`,
        );
  ...
  return (
    <>
              ...
              <RadioGroup value={callType} onValueChange={setCallType}>
                {[
                  ...
                  },
                  {
                    ...
                    description:
                      "Preparation of Quotation with a special pricing offer.",
                  },
                  {
          ...
          <FieldGroup>
            <FieldSet>
              {followUpDate ? (
                <Alert
                  variant="default"
                  className="mb-4 flex flex-col gap-3 border-cyan-300 border-3 bg-cyan-100"
                >
                  <div>
                    ...
                    <AlertDescription>
                      {followUpDate} — This is the scheduled date to reconnect
                      with the client.
                    </AlertDescription>
                  </div>
              ...
              >
                ...
                <SelectContent>
                  <SelectGroup>
                    ...
                    <SelectItem value="Declined / Dissaproved">
                      Declined / Dissaproved
                    </SelectItem>
                    <SelectItem value="Pending PD">Pending PD</SelectItem>
                    <SelectItem value="Pending Procurement">
                      Pending Procurement
                    </SelectItem>
                    <SelectItem value="Pending Client Approval">
                      Pending Client Approval
                    </SelectItem>
                    <SelectItem value="Wait Bid Results">
                      Wait Bid Results
                    </SelectItem>
          ...
          {showConfirmFollowUp && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 text-black">
              <div className="max-w-md rounded-none bg-white p-6 shadow-lg">
                <Alert
                  variant="default"
                  className="p-4 flex items-center gap-3 w-full min-w-[400px] rounded-none"
                >
                    ...
                    {isGenerating ? (
                      ...
                      <AlertDescription className="text-sm text-black">
                        ...
                        <strong className="text-lg">
                          {localQuotationNumber}
                        </strong>
                        <br />
                        <p className="mt-1 text-xs text-gray-600">
                          It is automatically generated based on the quotation
                          type, TSM prefix, current year, and a sequential
                          number.
                        </p>
                      </AlertDescription>
                    ) : (
                ...
                <div className="mt-4 flex flex-col gap-3">
                  <Button
                    ...
                    variant="outline"
                    className="w-full flex items-center justify-center gap-2 border border-dashed rounded-none p-10"
                  >
                    {isGenerating ? (
                      <>
                  ...
                  {!hasDownloaded && hasGenerated && (
                    <p
                      className="text-sm text-yellow-600 mt-2 border border-dashed p-2 bg-red-100"
                      hidden={false}
                    >
                      ⚠️ Please download the quotation before saving.
                      <span className="text-sm text-red-600 italic ml-1">
                        Note: If there are no products or the quotation is
                        empty, please do not download.
                      </span>
                    </p>
                  )}
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent
          className={`max-h-[90vh] overflow-y-auto p-6 transition-all duration-300 ${
            selectedProducts.length === 0 ? "w-[60vw]" : "w-[90vw]"
          }`}
          style={{
        ...
        >
          ...
          <div
            className={`grid gap-6 mt-4 max-h-[75vh] overflow-hidden ${
              selectedProducts.length === 0
                ? "grid-cols-1"
                : "grid-cols-[1fr_2.5fr]"
            }`}
              ...
              <div className="flex flex-col gap-4 sticky top-0 bg-white z-10 pb-2">
                <div className="flex border rounded-md overflow-hidden border-gray-300">
                  <button
                    ...
                    className={`flex-1 py-4 text-[10px] font-bold transition-colors ${
...
${spec.value}
                    ...
                    />
                    {isSearching && (
                      <p className="text-[10px] animate-pulse">Searching...</p>
                    )}
                  </>
                )}
              </div>
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-1 xl:grid-cols-2 gap-4">
                    {searchResults.map((item) => (
                      <Card
                        key={item.id}
                        className="cursor-pointer hover:bg-gray-50 rounded-xs"
                      >
                  ...
                  <table className="w-full text-xs table-auto border-collapse border border-gray-300">
                    <thead>
                      <tr className="bg-gray-100">
                        ...
                        <th className="border p-4 text-center w-10">
                          Subtotal
                        </th>
                    ...
                    <tbody>
                      {selectedProducts.map((p, idx) => {
                        ...
                        const baseAmount = p.price * p.quantity;
                        const discountedAmount = isDiscounted
                          ? (baseAmount * rowDiscount) / 100
                          : 0;
                        ...
                        <td className="border border-gray-300 p-2 text-center">
                          ...
                          {selectedProducts
                            .reduce((acc, p) => {
                              ...
                              const baseAmount = p.price * p.quantity;
                              return (
                                acc + baseAmount - (baseAmount * discount) / 100
                              );
                            }, 0)
                            .toFixed(2)}
                        </td>
          ...
          <div className="text-xs text-red-600 text-right italic border border-red-500 p-2 bg-red-100">
            <p>
              Note: Quotation Number is not included in the Preview Sample (only
              appears on the final downloaded quotation).
            </p>
          </div>
            ...
            return (
              <div className="flex flex-col bg-white min-h-full font-sans text-[#121212]">
                ...
                <div className="px-6 py-5 space-y-1">
                  ...
                  `}
                      >
                        <span className="col-span-1 font-black text-[10px] text-[#121212]">
                          {info.label}:
                        </span>
                        <span className="col-span-5 text-[11px] font-bold text-gray-700 pl-4">
                          {info.value || "---"}
                        </span>
                      </div>
                    ))}
                  ...
                  <p className="text-[10px] italic mt-5 text-gray-500 font-medium">
                    We are pleased to offer you the following products for
                    consideration:
                  </p>
                    ...
                    <table className="w-full text-[12px] border-collapse">
                      <thead>
                        <tr className="bg-[#F9FAFA] border-b border-black font-black uppercase text-[#121212]">
                          <th className="p-3 border-r border-black w-16 text-center">
                            ITEM NO
                          </th>
                          <th className="p-3 border-r border-black w-16 text-center">
                            QTY
                          </th>
                          <th className="p-3 border-r border-black w-32 text-center">
                            REFERENCE PHOTO
                          </th>
                          <th className="p-3 border-r border-black text-left">
                            PRODUCT DESCRIPTION
                          </th>
                          <th className="p-3 border-r border-black w-32 text-right">
                            UNIT PRICE
                          </th>
                      ...
                      <tbody className="divide-y divide-black">
                        {payload.items.map((item, idx) => (
                          ...
                          >
                            <td className="p-4 text-center border-r border-black align-top font-bold text-gray-400 capitalize">
                              <span className="font-bold text-black">
                                {item.itemNo}.
                              </span>{" "}
                              {item.itemRemarks}
                            </td>
                            <td className="p-4 text-center border-r border-black align-top font-black text-[#121212]">
                              {item.qty}
                            </td>
                            <td className="p-3 border-r border-black align-top bg-white">
                              {item.photo ? (
                                <img
                                  src={item.photo}
                                  className="w-24 h-24 object-contain mx-auto mix-blend-multiply"
                                  alt="sku-ref"
                                />
                              ) : (
                                <div className="w-24 h-24 bg-gray-50 flex items-center justify-center text-[8px] text-gray-300 italic">
                                  No Image
                                </div>
                              )}
                            </td>
                            <td className="p-4 border-r border-black align-top">
                              <p className="font-black text-[#121212] text-xs uppercase mb-1">
                                {item.title}
                              </p>
                              <p className="text-[9px] text-blue-600 font-bold mb-3 tracking-tighter">
                                {item.sku}
                              </p>
                              <div
                        ...
                        <tr className="border-t-2 border-black bg-gray-200 text-gray-900 h-[45px]">
                          ...
                          <td className="px-4 border-r border-gray-400">
                            <div className="flex gap-4 text-[12px] font-bold uppercase tracking-tight">
                              ...
                              <span
                                className={
                                  payload.vatTypeLabel === "Zero-Rated"
                                    ? "text-gray-900"
                                    : "text-gray-400"
                                }
                              >
                  ...
                  <div className="mt-4 border border-black text-[9.5px] leading-tight">
                    <div className="grid grid-cols-6 border-b border-black">
                      <div className="col-span-1 p-2 bg-yellow-400 font-black border-r border-black">
                        Included:
                      </div>
                      <div className="col-span-5 p-2 bg-yellow-100">
                        <p>
                          Orders Within Metro Manila: Free delivery for a
                          minimum sales transaction of ₱5,000.
                        </p>
                        <p>
                          Orders outside Metro Manila Free delivery is available
                          for a minimum sales transaction of ₱10,000 in Rizal,
                          ₱15,000 in Bulacan and Cavite, and ₱25,000 in Laguna,
                          Pampanga, and Batangas.
                        </p>
                      </div>
                    </div>
                    <div className="grid grid-cols-6 border-b border-black">
                      ...
                      <div className="col-span-5 p-2 bg-yellow-100">
                        <p>All lamp poles are subject to a delivery charge.</p>
                        <p>
                          Installation and all hardware/accessories not
                          indicated above.
                        </p>
                        <p>
                    ...
                    <div className="grid grid-cols-6 bg-yellow-50">
                      <div className="col-span-1 p-2 font-black border-r border-black">
                        Notes:
                      </div>
                      <div className="col-span-5 p-2 italic">
                        <p>
                          Deliveries are up to the vehicle unloading point only.
                        </p>
                        <p>
                          Additional shipping fee applies for other areas not
                          mentioned above.
                        </p>
                        <p>
                          Subject to confirmation upon getting the actual weight
                          and dimensions of the items.
                        </p>
                        <span className="font-black underline block mt-1 text-red-600">
                          In cases of client error, there will be a 10%
                          restocking fee for returns, refunds, and exchanges.
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-6 border-t-2 border-black pt-2">
                    <h3 className="bg-[#121212] text-white px-3 py-1 text-[10px] font-black inline-block mb-4 uppercase">
                      Terms and Conditions
                    </h3>

                    <div className="grid grid-cols-12 gap-y-4 text-[9px]">
                      <div className="col-span-2 font-black uppercase">
                        Availability:
                      </div>
                      <div className="col-span-10 pl-4 border-l border-gray-100 bg-yellow-50">
                        <p>
                          *5-7 days if on stock upon receipt of approved PO.
                        </p>
                        <p>
                          *For items not on stock/indent order, an estimate of
                          45-60 days upon receipt of approved PO & down payment.
                          Barring any delay in shipping and customs clearance
                          beyond Disruptive's control.
                        </p>
                        <p>
                          *In the event of a conflict or inconsistency in
                          estimated days under Availability and another estimate
                          indicated elsewhere in this quotation, the latter will
                          prevail.
                        </p>
                      </div>
                      ...
                      <div className="col-span-10 pl-4 border-l border-gray-100 bg-yellow-50">
                        <p>
                          One (1) year from the time of delivery for all busted
                          lights except the damaged fixture.
                        </p>
                        <p>
                        ...
                        <p>*If the unit is being tampered with.</p>
                        <p>
                          *If the item(s) is/are altered in any way by
                          unauthorized technicians.
                        </p>
                        <p>
                          *If it has been subjected to misuse, mishandling,
                          neglect, or accident.
                        </p>
                        <p>
                          *If damaged due to spillage of liquids, tear
                          corrosion, rusting, or stains.
                        </p>
                        <p>
                          *This warranty does not cover loss of product
                          accessories such as remote control, adaptor, battery,
                          screws, etc.
                        </p>
                        <p>
                          *Shipping costs for warranty claims are for customers'
                          account.
                        </p>
                        <p>
                          *If the product purchased is already phased out when
                          the warranty is claimed, the latest model or closest
                          product SKU will be given as a replacement.
                        </p>
                      </div>
                      ...
                      <div className="col-span-10 pl-4 border-l border-gray-100">
                        <p>
                          Sales order has{" "}
                          <span className="text-red-600 font-black italic">
                            validity period of 14 working days
                          </span>
                          . (excluding holidays and Sundays) from the date of
                          issuance. Any sales order not confirmed and no
                          verified payment within this{" "}
                          <span className="text-red-600 font-black">
                            14-day period will be automatically cancelled
                          </span>
                          .
                        </p>
                      </div>
                      ...
                      <div className="col-span-10 pl-4 border-l border-gray-100 bg-yellow-50">
                        <p>
                          Orders with confirmation/verified payment but
                          undelivered after 14 working days (excluding holidays
                          and Sundays starting from picking date) due to
                          clients’ request or shortcomings will be charged a
                          storage fee of 10% of the value of the orders per
                          month{" "}
                          <span className="text-red-600 font-black">
                            {" "}
                      ...
                      <div className="col-span-10 pl-4 border-l border-gray-100 bg-yellow-50">
                        <p>
                          ...
                          if the product received is defective, damaged, or
                          incomplete. This must be communicated to Disruptive,
                          and Disruptive has duly acknowledged communication as
                          received within a maximum of 7 days to qualify for
                          replacement.
                        </p>
                      </div>
                      ...
                      <div className="col-span-10 pl-4 border-l border-gray-100 ">
                        ...
                        <p>
                          <strong>
                            NOTE: Orders below 10,000 pesos can be paid in cash
                            at the time of delivery. Exceeding 10,000 pesos
                            should be transacted through bank deposit or mobile
                            electronic transactions.
                          </strong>
                        </p>
                        <p>
                          For special items, Seventy Percent (70%) down payment,
                          30% upon delivery.
                        </p>
                        <p className="mt-5">
                          <b>BANK DETAILS</b>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            <p>
                              Account Number:{" "}
                              {isEcoshift ? "0021-8801-7271" : "0021-8801-9258"}
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="col-span-10 pl-4 border-l border-gray-100">
                        <p className="text-red-600 font-black underline">
                          Thirty (30) calendar days from the date of this offer.
                        </p>
                        <p>
                          In the event of changes in prevailing market
                          conditions, duties, taxes, and all other importation
                          charges, quoted prices are subject to change.
                        </p>
                      </div>
                      ...
                      <div className="col-span-10 pl-4 border-l border-gray-100 bg-yellow-50">
                        <p>1. Above quoted items are non-cancellable.</p>
                        <p>
                          2. If the customer cancels the order under any
                          circumstances, the client shall be responsible for
                          100% cost incurred by Disruptive, including freight
                          and delivery charges.
                        </p>
                        <p>
                          3. Downpayment for items not in stock/indent and
                          order/special items are non-refundable and will be
                          forfeited if the order is canceled.
                        </p>
                        <p>
                          4. COD transaction payments should be ready upon
                          delivery. If the payment is not ready within seven (7)
                          days from the date of order, the transaction is
                          automatically canceled.
                        </p>
                        <p>
                          5. Cancellation for Special Projects (SPF) are not
                          allowed and will be subject to a 100% charge.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-12 pt-4 border-t-4 border-blue-700 pb-20">
                    <p className="text-[9px] mb-8 font-medium">
                      Thank you for allowing us to service your requirements. We
                      hope that the above offer merits your acceptance. Unless
                      otherwise indicated, you are deemed to have accepted the
                      Terms and Conditions of this Quotation.
                    </p>
                    ...
                    <div className="grid grid-cols-2 gap-x-20 gap-y-12">
                      ...
                      <div className="space-y-10">
                        <div>
                          <p className="italic text-[10px] font-black mb-10">
                            {isEcoshift
                              ? "Ecoshift Corporation"
                              : "Disruptive Solutions Inc"}
                          </p>
                          ...
                          <p className="text-[9px] text-gray-500 font-bold italic">
                            Mobile: {payload.salescontact || "N/A"}
                          </p>
                          <p className="text-[9px] text-gray-500 font-bold italic">
                            Email: {payload.salesemail || "N/A"}
                          </p>
                        </div>

                        <div>
                          <p className="text-[10px] font-black uppercase text-gray-400 mb-10">
                            Approved By:
                          </p>
                          <p className="text-[11px] font-black uppercase mt-1">
                          ...
                          <p className="text-[9px] text-gray-500 font-bold italic">
                            Mobile: {payload.tsmDetails?.contact || "N/A"}
                          </p>
                          <p className="text-[9px] text-gray-500 font-bold italic">
                            Email: {payload.tsmDetails?.email || "N/A"}
                          </p>
                        </div>
                      ...
                      <div className="space-y-10 flex flex-col items-end">
                        <div className="w-64">
                          ...
                          <p className="text-[9px] text-center font-bold text-gray-500 mt-1 uppercase tracking-widest">
